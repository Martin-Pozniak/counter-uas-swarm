...
class DroneBrain:
    ...
    def neighbor_global_pos_changed_callback(self, position_object, index):
        ...
        for i in range(1, len(self.neighbor_global_position_objects)+2):
            if(str(i) in self.neighbor_global_position_objects.keys()): 
                ...
                print " D" + str(i) + ": Abs:" + str(absolute) + " Hor:" + str(horiz) + " Vert:" + str(vert) + " Brng:" + str(bearing)

        # else:
        #     self.count += 1

        self.compute_conflict_state(self.neighbor_global_position_objects[index], index)

        if (self.conflict_state["xy"] != "xy-free" or self.conflict_state["z"] != "z-free"):
            self.currently_avoiding = True
            self.avoid_collision(self.neighbor_global_position_objects[index], self.conflict_state, index)
    ...
    def compute_conflict_state(self, other_pos_object, i):    
        ...
        if(horizontal_distance < self.reserved_cyl_radius and vertical_distance <= self.reserved_cyl_vertical):
            # xz conflict
            # If there exists an escape angle
            self.conflict_state['xy'] = 'rendezvous'
            # Else
            self.conflict_state['xy'] = 'xy-blocked'
        if(horizontal_distance < self.reserved_cyl_radius and vertical_distance > self.reserved_cyl_vertical and vertical_distance < self.blocking_cyl_vertical):
    ...
    def avoid_collision(self, other_global_pos_object, conflict_state, index):
